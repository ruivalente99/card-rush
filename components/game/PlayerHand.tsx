...
export const PlayerHand = forwardRef<HTMLDivElement, PlayerHandProps>(
  function PlayerHand({ player, isActive, deck, size = 'md', emoji }, ref) {
    ...
    const liveScore = rs.busted ? 0 : calculateRoundScore(rs.hand, rs.isLucky7, rs.hyperTrainActive);
    ...
    return (
      ...
      >
        {/* HyperTrain active bar */}
        {rs.hyperTrainActive && (
          <div className="hypertrain-active-bar h-1.5 rounded-full mb-2" />
        )}
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {rs.isLucky7 && (
              <Badge className="bg-yellow-500 text-black text-xs font-bold px-1.5 animate-bounce">Lucky 7! 🍀</Badge>
            )}
            {rs.hyperTrainActive && !rs.isLucky7 && (
              <Badge className="text-xs font-bold px-1.5 border-white/30 card-hypertrain">🌈 ×2</Badge>
            )}
          </div>
          <div className="text-right shrink-0">
            <div className="text-muted-foreground text-xs">Round</div>
            <div className={`font-bold text-sm ${rs.busted ? 'text-red-400 line-through' : rs.hyperTrainActive ? 'text-yellow-400' : 'text-foreground'}`}>
              {liveScore}
            </div>
          </div>
        </div>
